...
// <---------------- CHANGED
// MKUserLocationView  is only supported on: 'ios' 14.0 and later, 'maccatalyst' 14.0 and later, 'macOS/OSX' 11.0 and later, 'tvos' 14.0 and later.         
        
        //private static MKAnnotationView GetViewForAnnotations(MKMapView mapView, IMKAnnotation annotation)
        //{
        //    MKAnnotationView annotationView;
        //    if (annotation is Platforms.iOS.CustomAnnotation customAnnotation)
        //    {
        ...
        private static MKAnnotationView GetViewForAnnotations(MKMapView mapView, IMKAnnotation annotation)
        {
            ...
            {
                if (UIDevice.CurrentDevice.CheckSystemVersion(14, 0)) // Проверка версии iOS
                {
                    annotationView = new MKUserLocationView(annotation, null);
                }
                else
                {
                    // Если версия iOS меньше 14.0, можете предпринять другие действия или вернуть другой тип аннотации
                    // Например:
                    annotationView = new MKAnnotationView(annotation, "fallbackIdentifier");
                }
            }
        ...
        private void AddPins(IEnumerable<IMapPin> mapPins)
        {
            ...
            foreach (var pin in mapPins)
            {
                ...
                if (pinHandler is IMapPinHandler mapPinHandler)
                {
                    ...
                    if (pin is MVVM.MAP.Model.CustomPin cp)
                    {
                        cp.ImageSource.LoadImage(MauiContext, result =>
                        {
                            markerOption = new Platforms.iOS.CustomAnnotation()
                            {
                                Identifier = cp.Id,
                                Image = result?.Value,
                                Title = pin.Label,
